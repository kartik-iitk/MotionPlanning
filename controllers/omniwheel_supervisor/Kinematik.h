...
class RobotKinematic {
   public:
    ...
    RobotKinematic(){};
    Point2D getPos();
    void ForwardKinematic(Point2D &outForward, float s1, float s2, float s3,
                          float s4);
    void CalculateOdometry(const double yaw);
    void inversKinematic(motion &outputInvers, float vx, float vy, float theta,
                         const double ori);
   ...
   private:
    ...
    float L =
        20.8;  // radius from center to wheel (cm) //baseLength 2397.97 MM;
